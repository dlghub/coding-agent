...
class Workspace:
    ...
    def __init__(self, root: str | Path) -> None:
        ...
        if not self.root.exists():
            raise WorkspaceViolation(f"工作区根目录不存在: {self.root}")

        if not self.root.is_dir():
            raise WorkspaceViolation(f"工作区根目录不是文件夹: {self.root}")
    ...
    def resolve(self, user_path: str) -> Path:
        ...
        tartget = (self.root / candidate).resolve()

        try:
            tartget.relative_to(self.root)
        except ValueError as error:
            raise WorkspaceViolation(
                f"拒绝访问工作区外的路径: {user_path}"
            ) from error
